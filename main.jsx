...
const G = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  @keyframes borderGlow{0%{opacity:.5;background-position:0% 50%}50%{opacity:1;background-position:100% 50%}100%{opacity:.5;background-position:0% 50%}}
...
function GoldBorderFrame() {
  ...
  const offset = `${-scrollPct * 300}%`;
  const borderStyle = (edge) => {
    const base = {
      position:"fixed", zIndex:999, pointerEvents:"none",
      background:`linear-gradient(90deg, transparent, ${GOLD}, #f5d980, ${GOLD}, transparent)`,
      backgroundSize:"300% 100%",
      animation:"borderScroll 3s linear infinite",
      opacity: 0.6 + scrollPct * 0.4,
    };
    if (edge === "top")    return { ...base, top:0, left:0, right:0, height:2 };
    if (edge === "bottom") return { ...base, bottom:0, left:0, right:0, height:2 };
    if (edge === "left")   return { ...base, top:0, bottom:0, left:0, width:2, background:`linear-gradient(180deg, transparent, ${GOLD}, #f5d980, ${GOLD}, transparent)`, backgroundSize:"100% 300%", animation:"none", backgroundPositionY: offset };
    if (edge === "right")  return { ...base, top:0, bottom:0, right:0, width:2, background:`linear-gradient(180deg, transparent, ${GOLD}, #f5d980, ${GOLD}, transparent)`, backgroundSize:"100% 300%", animation:"none", backgroundPositionY: offset };
  };
  return (
    <>
        ...
        <div onClick={() => go("home")} style={{ cursor:"pointer", display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ position:"relative" }}>
            ...
            <img
              src="/logo.png"
              alt="TruPep"
              style={{ width:44, height:44, objectFit:"contain", position:"relative", animation:"float 3.5s ease-in-out infinite" }}
            />
          </div>
          <div>
            <div style={{ fontFamily:SERIF, fontSize:18, fontWeight:300, color:GOLD, letterSpacing:3, lineHeight:1 }}>TruPep</div>
    ...
    <div style={{ padding:"100px 24px 80px" }}>
      <div style={{ maxWidth:1160, margin:"0 auto" }}>
        <div style={{ textAlign:"center", marginBottom:60 }}>
          <div style={{ fontSize:9, letterSpacing:4, color:GOLD, textTransform:"uppercase", marginBottom:12, animation:"shimmer 3s ease-in-out infinite" }}>Research Protocols</div>
          <h1 style={{ fontFamily:SERIF, fontSize:"clamp(32px,5vw,60px)", fontWeight:300, marginBottom:16 }}>Curated Peptide Stacks</h1>
          <p style={{ fontSize:13, color:DIM, maxWidth:540, margin:"0 auto", lineHeight:1.9 }}>
            Each stack is a curated combination targeting a specific research outcome. All compounds are available individually in the product catalog.
          </p>
        </div>
            ...
            return (
              <div key={s.id}
                style={{
                  ...
                  boxShadow: isOpen ? `0 0 28px rgba(201,168,76,0.18)` : "none",
                  cursor:"pointer",
                }}
                onClick={() => setActive(isOpen ? null : s.id)}
                onMouseEnter={e => { if (!isOpen) e.currentTarget.style.borderColor = `rgba(201,168,76,0.55)`; }}
                onMouseLeave={e => { if (!isOpen) e.currentTarget.style.borderColor = BORDER; }}
              >
                {/* Stack header */}
                <div style={{ padding:"28px 28px 24px" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                    <span style={{ fontSize:34, lineHeight:1 }}>{s.icon}</span>
                    <span style={{ fontSize:16, color: isOpen ? GOLD : DIM, transition:"color .2s" }}>{isOpen ? "−" : "+"}</span>
                  </div>
                  <div style={{ fontSize:8, letterSpacing:3, color:GOLD, textTransform:"uppercase", marginTop:14, marginBottom:5 }}>{s.tagline}</div>
                  <div style={{ fontFamily:SERIF, fontSize:22, marginBottom:10 }}>{s.name}</div>
                  <p style={{ fontSize:11, color:DIM, lineHeight:1.75 }}>{s.desc}</p>
                </div>

                {/* Compound list — toggles open */}
                <div style={{
                  overflow:"hidden",
                  maxHeight: isOpen ? "600px" : "0",
                  transition:"max-height .35s ease",
                }}>
                  <div style={{ borderTop:`1px solid ${BORDER}`, padding:"20px 28px 24px" }}>
                    <div style={{ fontSize:8, letterSpacing:2, color:GOLD, textTransform:"uppercase", marginBottom:14 }}>Stack Compounds</div>
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px 16px", marginBottom:20 }}>
                      {s.peptides.map(name => {
                        const p = PRODUCTS.find(pr => pr.name === name || pr.name.startsWith(name.split(" ")[0]));
                        return (
                          <div key={name} style={{ display:"flex", alignItems:"center", gap:8 }}>
                            <span style={{ color:GOLD, fontSize:6, flexShrink:0 }}>◆</span>
                            <div>
                              <div style={{ fontSize:11, lineHeight:1.3 }}>{name}</div>
                              {p && <div style={{ fontSize:9, color:GOLD }}>{p.variants.map(v => `${v.label} — $${v.price}`).join(" / ")}</div>}
                            </div>
                          </div>
                        );
                      })}
                    </div>
                    <button onClick={e => { e.stopPropagation(); setPage("products"); }} style={{
                      width:"100%", padding:"10px 0", border:`1px solid ${GOLD}`,
                      color:GOLD, fontSize:9, letterSpacing:2, textTransform:"uppercase",
        ...
        {/* Bottom CTA */}
        <div style={{ textAlign:"center", padding:"44px 32px", border:`1px solid ${BORDER}`, position:"relative", overflow:"hidden" }}>
          ...
          <div style={{ fontFamily:SERIF, fontSize:"clamp(22px,3vw,32px)", marginBottom:10, position:"relative" }}>Need a Custom Protocol?</div>
          <p style={{ fontSize:12, color:DIM, marginBottom:28, lineHeight:1.8, maxWidth:440, margin:"0 auto 28px" }}>
