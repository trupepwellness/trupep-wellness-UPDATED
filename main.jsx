...
const G = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  @keyframes floatMirror{0%,100%{transform:translateY(0)}50%{transform:translateY(-7px)}}
  @keyframes borderScroll{0%{background-position:0% 0%}100%{background-position:200% 0%}}
  @keyframes pulseGlow{0%,100%{box-shadow:0 0 12px ${GOLD_GLOW}}50%{box-shadow:0 0 28px rgba(201,168,76,0.55)}}
  @keyframes particle{0%{transform:translateY(0) translateX(0);opacity:0}10%{opacity:1}90%{opacity:.4}100%{transform:translateY(-120vh) translateX(var(--dx,20px));opacity:0}}
  ...
  .card-hover:hover{transform:translateY(-5px);box-shadow:0 12px 40px rgba(201,168,76,0.18);border-color:${GOLD}!important}
...
function TrustBar() {
  ...
  return (
    <div style={{ borderTop:`1px solid ${BORDER}`, borderBottom:`1px solid ${BORDER}`, padding:"18px 24px", background:`linear-gradient(90deg, rgba(201,168,76,.04), rgba(201,168,76,.08), rgba(201,168,76,.04))` }}>
      <div style={{ maxWidth:1100, margin:"0 auto", display:"flex", flexWrap:"wrap", justifyContent:"center", gap:"14px 36px" }}>
        {items.map(({ icon, label }) => (
          <div key={label} style={{ display:"flex", alignItems:"center", gap:7 }}>
            <span style={{ fontSize:14 }}>{icon}</span>
            <span style={{ fontSize:9, letterSpacing:2, textTransform:"uppercase", color:DIM, fontWeight:600 }}>{label}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ position:"absolute", inset:0, overflow:"hidden", pointerEvents:"none", zIndex:0 }}>
      {particles.map(i => {
        ...
        const delay  = `${(i * 1.3) % 8}s`;
        const dur    = `${7 + (i * 0.7) % 8}s`;
        const size   = 2 + (i % 3);
        const dx     = `${-30 + (i * 37 % 60)}px`;
        return (
          <div key={i} style={{
            position:"absolute", bottom:"-10px", left,
            width:size, height:size, borderRadius:"50%",
            background: i % 3 === 0 ? GOLD : i % 3 === 1 ? GOLD2 : "rgba(201,168,76,0.5)",
...
function FloatingHeroLogo() {
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", marginBottom:36 }}>
      {/* Primary logo — floats upward */}
      <div style={{ animation:"float 3.5s ease-in-out infinite", position:"relative", zIndex:2 }}>
        <div style={{
        ...
        <img
          ...
          style={{
            ...
            filter:`drop-shadow(0 0 22px rgba(201,168,76,0.65)) drop-shadow(0 0 8px rgba(201,168,76,0.35))`,
          }}
        />
      </div>
      {/* Mirror reflection — scaleY flipped wrapper, inner div animates inverse */}
      <div style={{ transform:"scaleY(-1)", marginTop:1, lineHeight:0, display:"block" }}>
        <div style={{ animation:"floatMirror 3.5s ease-in-out infinite" }}>
          <img
            ...
            aria-hidden="true"
            style={{
              width:108, height:108, objectFit:"contain", display:"block",
              maskImage:"linear-gradient(to bottom, rgba(0,0,0,0.4) 0%, transparent 68%)",
              WebkitMaskImage:"linear-gradient(to bottom, rgba(0,0,0,0.4) 0%, transparent 68%)",
...
const sectionHead = (label, title, sub) => (
  <div className="reveal" style={{ textAlign:"center", marginBottom:52 }}>
    <div style={{ fontSize:9, letterSpacing:5, color:GOLD, textTransform:"uppercase", marginBottom:14, animation:"shimmer 3s ease-in-out infinite" }}>{label}</div>
    <h1 style={{ fontFamily:SERIF, fontSize:"clamp(30px,5vw,56px)", fontWeight:300, marginBottom: sub ? 14 : 0 }}>{title}</h1>
    {sub && <p style={{ fontSize:13, color:DIM, maxWidth:520, margin:"0 auto", lineHeight:1.9 }}>{sub}</p>}
  </div>
);
...
function GoldBorderFrame() {
  ...
  const vert = (side) => ({
    position:"fixed", top:0, bottom:0, [side]:0, width:2, zIndex:999, pointerEvents:"none",
    ...
    opacity: 0.55 + scrollPct * 0.45,
    transition:"background .1s",
  });
  const horiz = {
    position:"fixed", left:0, right:0, height:2, zIndex:999, pointerEvents:"none",
...
function Nav({ count, onCart, setPage, page }) {
  ...
  return (
    <nav style={{
      ...
      {/* Mobile menu */}
      {mobile && open && (
        <div style={{ background:"rgba(14,14,14,1)", borderTop:`1px solid rgba(201,168,76,0.35)` }}>
          {LINKS.map(([l, id]) => (
            <button key={id} onClick={() => go(id)} style={{
              display:"block", width:"100%", padding:"16px 28px", textAlign:"left",
              fontSize:13, letterSpacing:2.5, textTransform:"uppercase", fontWeight:600,
              color: page === id ? GOLD : "rgba(255,255,255,0.88)",
              background: page === id ? "rgba(201,168,76,0.07)" : "none",
              border:"none", borderBottom:`1px solid rgba(201,168,76,0.14)`,
              cursor:"pointer", transition:"color .2s, background .2s",
            }}
              onMouseEnter={e => { e.currentTarget.style.color = GOLD; e.currentTarget.style.background = "rgba(201,168,76,0.07)"; }}
              onMouseLeave={e => { e.currentTarget.style.color = page === id ? GOLD : "rgba(255,255,255,0.88)"; e.currentTarget.style.background = page === id ? "rgba(201,168,76,0.07)" : "none"; }}
            >{l}</button>
          ))}
        </div>
      )}
      ...
      <div style={{ maxWidth:1200, margin:"0 auto" }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))", gap:44, marginBottom:48 }}>
          <div>
            ...
            <div style={{ fontSize:11, color:"rgba(255,255,255,0.75)", lineHeight:1.9 }}>Miami's premier advanced peptide wellness source. Research compounds for the serious professional.</div>
          </div>
          <div>
            <div style={{ fontSize:9, letterSpacing:3, color:GOLD, textTransform:"uppercase", marginBottom:16 }}>Navigate</div>
            {LINKS.map(([l, id]) => (
              <button key={id} onClick={() => setPage(id)} style={{ display:"block", padding:"6px 0", color:"rgba(255,255,255,0.72)", fontSize:12, background:"none", border:"none", cursor:"pointer", letterSpacing:1, transition:"color .2s" }}
                onMouseEnter={e => e.currentTarget.style.color = GOLD}
          ...
          <div>
            ...
            <div style={{ fontSize:12, color:"rgba(255,255,255,0.78)", lineHeight:2.4 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:14, paddingTop:8 }}>
          <p style={{ fontSize:11, color:"rgba(255,255,255,.58)", lineHeight:1.8, maxWidth:700 }}>
          ...
          <div style={{ fontSize:11, color:"rgba(255,255,255,.5)" }}>© {new Date().getFullYear()} TruPep Wellness</div>
        </div>
      </div>
    </footer>
  );
...
function ProductCard({ product, onAdd }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ position:"absolute", top:0, right:0, width:60, height:60, background:`radial-gradient(circle at top right, rgba(201,168,76,0.12), transparent)`, pointerEvents:"none" }} />

      <div style={{ fontSize:8, letterSpacing:3, color:GOLD, textTransform:"uppercase", marginBottom:8 }}>{product.cat}</div>
      <div style={{ fontFamily:SERIF, fontSize:19, marginBottom:9 }}>{product.name}</div>
      <div style={{ fontSize:11, color:DIM, lineHeight:1.8, flex:1, marginBottom:18 }}>{product.desc}</div>

      {product.variants.length > 1 && (
        <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:14 }}>
          {product.variants.map((vv, i) => (
            <button key={i} onClick={() => setSel(i)} style={{
              padding:"5px 13px", fontSize:9, letterSpacing:1, borderRadius:20,
              border:`1px solid ${i === sel ? GOLD : BORDER}`,
              color: i === sel ? GOLD : DIM2,
              background: i === sel ? "rgba(201,168,76,0.1)" : "none",
              transition:"all .2s",
            }}>{vv.label}</button>
          ))}
        </div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        ...
        <button onClick={handleAdd} className="btn-hover" style={{
          padding:"8px 18px", fontSize:9, letterSpacing:2, textTransform:"uppercase",
...
function HomePage({ setPage, onAdd }) {
  ...
  return (
    <>
      {/* Hero */}
      <div style={{ padding:"130px 24px 90px", textAlign:"center", position:"relative", overflow:"hidden" }}>
        <FloatingParticles />
      ...
      <div className="reveal" style={{ padding:"52px 24px" }}>
        <div style={{ maxWidth:900, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:24, textAlign:"center" }}>
          {[["25+","Compounds"],["7","Protocol Stacks"],["Miami","Based"],["Free","Consultation"]].map(([n, l]) => (
            <div key={l} style={{ padding:"24px 16px", border:`1px solid ${BORDER}`, borderRadius:6, background:GLASS }}>
              <div style={{ fontFamily:SERIF, fontSize:30, color:GOLD, marginBottom:6 }}>{n}</div>
      ...
      <div style={{ padding:"20px 24px 80px" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div className="reveal" style={{ textAlign:"center", marginBottom:48 }}>
            <div style={{ fontSize:9, letterSpacing:5, color:GOLD, textTransform:"uppercase", marginBottom:12 }}>Featured</div>
      ...
      {/* Testimonials */}
      <div style={{ padding:"60px 24px 80px", background:"rgba(5,5,5,.6)" }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(270px,1fr))", gap:22 }}>
            {TESTIMONIALS.map((t, i) => (
              <div key={i} className="card-hover reveal"
                style={{ padding:28, border:`1px solid ${BORDER}`, borderRadius:6, background:GLASS2 }}>
                <div style={{ color:GOLD, marginBottom:11, fontSize:15, letterSpacing:2 }}>{"★".repeat(t.stars)}</div>
                <p style={{ fontSize:12, color:DIM, lineHeight:1.9, marginBottom:16 }}>"{t.text}"</p>
                <div style={{ fontSize:10, letterSpacing:2, color:DIM2 }}>— {t.name}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="reveal" style={{ padding:"80px 24px", textAlign:"center", borderTop:`1px solid ${BORDER}`, position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ maxWidth:580, margin:"0 auto", position:"relative" }}>
          ...
          <p style={{ fontSize:13, color:DIM, lineHeight:1.9, marginBottom:36 }}>Use our peptide calculator to determine your exact dosing, or reach out for a free consultation.</p>
      ...
      <div style={{ maxWidth:1160, margin:"0 auto" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))", gap:24, marginBottom:60 }}>
          {STACKS.map(s => {
            ...
            return (
              <div key={s.id} className={isOpen ? "" : "card-hover"}
                style={{
              ...
              >
                <div style={{ padding:"28px 28px 24px" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                    ...
                    <div style={{ fontSize:18, color: isOpen ? GOLD : DIM2, transition:"color .2s", fontWeight:300 }}>{isOpen ? "−" : "+"}</div>
                  </div>
                  <div style={{ fontSize:8, letterSpacing:3, color:GOLD, textTransform:"uppercase", marginTop:16, marginBottom:6 }}>{s.tagline}</div>
                  <div style={{ fontFamily:SERIF, fontSize:23, marginBottom:10 }}>{s.name}</div>
                  <p style={{ fontSize:11, color:DIM, lineHeight:1.8 }}>{s.desc}</p>
                </div>
                ...
                <div style={{ overflow:"hidden", maxHeight: isOpen ? "700px" : "0", transition:"max-height .4s ease" }}>
                  <div style={{ borderTop:`1px solid ${BORDER}`, padding:"20px 28px 28px" }}>
                    <div style={{ fontSize:8, letterSpacing:2, color:GOLD, textTransform:"uppercase", marginBottom:16 }}>Stack Compounds</div>
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px 18px", marginBottom:22 }}>
                      {s.peptides.map(name => {
                        ...
                        return (
                          <div key={name} style={{ display:"flex", alignItems:"flex-start", gap:8 }}>
                            <span style={{ color:GOLD, fontSize:6, flexShrink:0, marginTop:4 }}>◆</span>
                            <div>
                              <div style={{ fontSize:11, lineHeight:1.4 }}>{name}</div>
                              {p && <div style={{ fontSize:9, color:GOLD, marginTop:2 }}>{p.variants.map(v => `${v.label} — $${v.price}`).join(" · ")}</div>}
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <button onClick={e => { e.stopPropagation(); setPage("products"); }} className="btn-hover"
                      style={{
                        ...
                        border:`1px solid ${GOLD}`,
                        color:GOLD, fontSize:9, letterSpacing:2, textTransform:"uppercase", borderRadius:4,
                        transition:"all .2s",
        ...
        <div className="reveal" style={{ textAlign:"center", padding:"48px 36px", border:`1px solid ${BORDER}`, borderRadius:8, background:GLASS, position:"relative", overflow:"hidden" }}>
          ...
          <p style={{ fontSize:12, color:DIM, marginBottom:30, lineHeight:1.85, maxWidth:440, margin:"0 auto 30px" }}>
    ...
    <div style={{ padding:"100px 24px 80px" }}>
      <div style={{ maxWidth:1020, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:60, alignItems:"start" }}>
        <div className="reveal">
          <div style={{ fontSize:9, letterSpacing:4, color:GOLD, textTransform:"uppercase", marginBottom:16 }}>Get In Touch</div>
          <h1 style={{ fontFamily:SERIF, fontSize:"clamp(28px,4vw,50px)", fontWeight:300, marginBottom:24 }}>Free Consultation</h1>
          <p style={{ fontSize:13, color:DIM, lineHeight:1.95, marginBottom:38 }}>Questions about compounds, your research protocol, or general inquiries — we're here to help.</p>
          {[["Email","trupepwellness@gmail.com"],["Phone","423-444-3668"],["Location","Miami, Florida"]].map(([k, v]) => (
            <div key={k} style={{ marginBottom:20, paddingBottom:20, borderBottom:`1px solid ${BORDER}` }}>
              <div style={{ fontSize:9, letterSpacing:3, color:GOLD, textTransform:"uppercase", marginBottom:5 }}>{k}</div>
    ...
    <div style={{ padding:"100px 24px 80px" }}>
      <div style={{ maxWidth:820, margin:"0 auto" }}>
        <div style={{ marginBottom:52 }}>
          <div style={{ fontSize:9, letterSpacing:4, color:GOLD, textTransform:"uppercase", marginBottom:12 }}>TruPep Wellness · Legal</div>
          <h1 style={{ fontFamily:SERIF, fontSize:"clamp(28px,5vw,50px)", fontWeight:300, marginBottom:8 }}>{title}</h1>
          <div style={{ fontSize:10, color:"rgba(255,255,255,.25)" }}>Last updated: May 2025</div>
        </div>
        {sections.map(([heading, body]) => (
          <div key={heading} style={{ marginBottom:28, paddingBottom:28, borderBottom:`1px solid ${BORDER}` }}>
            <h2 style={{ fontFamily:SERIF, fontSize:19, color:GOLD, fontWeight:300, marginBottom:11 }}>{heading}</h2>
            <p style={{ fontSize:12, color:DIM, lineHeight:1.95 }}>{body}</p>
          </div>
        ))}
      </div>
    </div>
