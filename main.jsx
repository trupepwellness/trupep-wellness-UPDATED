      ...
      <div style={{ maxWidth:1000, margin:"0 auto", padding:"60px 40px" }}>
        ...
        <div style={{ marginBottom:80 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))", gap:16 }}>
            {[
              { title:"How to Reconstitute Peptides", url:"https://www.youtube.com/watch?v=IkH5lHH-p3E", desc:"Step-by-step vial reconstitution with BAC water" },
              { title:"Subcutaneous Injection Guide", url:"https://www.youtube.com/watch?v=tFDJMhMGBRA", desc:"Safe injection technique for peptides" },
              { title:"Peptide Storage & Handling", url:"https://www.youtube.com/watch?v=BI7SyLHGMw0", desc:"Proper storage to protect peptide integrity" },
            ].map(v=>(
              <a key={v.title} href={v.url} target="_blank" rel="noreferrer" style={{ textDecoration:"none" }}>
                <div style={{ background:SURFACE2, border:`1px solid rgba(201,168,76,0.12)`, padding:"24px", transition:"all 0.3s", cursor:"pointer" }}
                  ...
                  <div style={{ fontSize:11, color:"rgba(255,255,255,0.45)", lineHeight:1.6 }}>{v.desc}</div>
...
function AgeGate({ onConfirm }) {
  return (
    <div style={{position:"fixed",inset:0,zIndex:9999,background:"rgba(5,5,5,0.97)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{maxWidth:480,width:"100%",background:SURFACE,border:`1px solid rgba(201,168,76,0.35)`,padding:"52px 40px",textAlign:"center",boxShadow:"0 40px 120px rgba(0,0,0,0.9)"}}>
        <div style={{height:2,background:`linear-gradient(90deg,transparent,${GOLD},transparent)`,marginBottom:36,marginLeft:-40,marginRight:-40}}/>
        ...
        <p style={{fontSize:13,color:"rgba(255,255,255,0.7)",lineHeight:1.9,marginBottom:28,fontWeight:300}}>
          This website contains information about research peptide compounds intended for adults <strong style={{color:WHITE}}>18 years of age or older</strong>. By entering you confirm you are 18+ and agree that all products are for <strong style={{color:WHITE}}>research and educational purposes only</strong> — not for human consumption or medical treatment.
        </p>
        <button onClick={onConfirm} style={{width:"100%",padding:"16px",background:`linear-gradient(135deg,${GOLD_DARK},${GOLD},${GOLD_LIGHT})`,color:BLACK,fontSize:11,fontWeight:700,letterSpacing:3,border:"none",cursor:"pointer",fontFamily:"'Montserrat',sans-serif",marginBottom:16}}>
          I AM 18+ — ENTER SITE
        </button>
        <p style={{fontSize:9,color:"rgba(255,255,255,0.25)",letterSpacing:0.5,lineHeight:1.8}}>
          By entering you acknowledge these products are not intended to diagnose, treat, cure, or prevent any disease. Not FDA evaluated. Consult a healthcare professional before use.
        </p>
        <div style={{height:1,background:`linear-gradient(90deg,transparent,rgba(201,168,76,0.2),transparent)`,marginTop:28,marginLeft:-40,marginRight:-40}}/>
      </div>
    </div>
  );
}
...
function CookieBanner({ onAccept }) {
  return (
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:8000,background:"rgba(10,10,10,0.97)",borderTop:`1px solid rgba(201,168,76,0.2)`,padding:"14px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,flexWrap:"wrap"}}>
      <p style={{fontSize:11,color:"rgba(255,255,255,0.6)",lineHeight:1.6,flex:1,minWidth:240}}>
        🍪 We use cookies to improve your experience. By continuing you accept our <span style={{color:GOLD,cursor:"pointer"}}>Privacy Policy</span>. Products are for research purposes only.
      </p>
      <div style={{display:"flex",gap:10,flexShrink:0}}>
        <button onClick={onAccept} style={{padding:"8px 20px",background:`linear-gradient(135deg,${GOLD_DARK},${GOLD})`,color:BLACK,fontSize:9,fontWeight:700,letterSpacing:2,border:"none",cursor:"pointer",fontFamily:"'Montserrat',sans-serif"}}>Accept</button>
        <button onClick={onAccept} style={{padding:"8px 16px",background:"transparent",border:"1px solid rgba(255,255,255,0.15)",color:GRAY,fontSize:9,letterSpacing:1,cursor:"pointer",fontFamily:"'Montserrat',sans-serif"}}>Decline</button>
      </div>
    </div>
  );
}
    ...
    <div style={{paddingTop:80,background:BLACK,minHeight:"100vh"}}>
      <div style={{padding:"60px 40px 40px",background:`linear-gradient(180deg,${SURFACE} 0%,${BLACK} 100%)`}}>
        <div style={{maxWidth:800,margin:"0 auto"}}>
          ...
          <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(28px,4vw,48px)",fontWeight:300,letterSpacing:-0.5,marginBottom:12}}>{title}</h1>
          <div style={{fontSize:10,color:GRAY,lineHeight:1.7}}>
            Last updated: {lastUpdated} &nbsp;·&nbsp;
            <span style={{color:"rgba(255,120,120,0.8)"}}>⚠ These pages are provided as a starting point. Have an attorney review your final legal documents.</span>
          </div>
        </div>
      </div>
      <div style={{maxWidth:800,margin:"0 auto",padding:"48px 40px",fontSize:13,lineHeight:1.9,color:"rgba(255,255,255,0.82)",fontWeight:300}}>
        {children}
      </div>
    </div>
  );
...
function LegalSection({ title, children }) {
  return (
    <div style={{marginBottom:32}}>
      <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:400,color:GOLD,marginBottom:10}}>{title}</h2>
      {children}
    </div>
  );
}
  ...
  return (
    <LegalPage title="Privacy Policy" lastUpdated="May 2025">
      <LegalSection title="1. Information We Collect">
        <p>We collect information you voluntarily provide when placing orders or contacting us, including your name, email address, phone number, and shipping address. We also collect standard web usage data through analytics.</p>
      </LegalSection>
      <LegalSection title="2. How We Use Your Information">
        <p>Your information is used solely to process orders, respond to inquiries, send order confirmations, and improve our services. We do not sell, rent, or share your personal information with third parties for marketing purposes.</p>
      </LegalSection>
      <LegalSection title="3. Data Security">
        <p>All form submissions are processed through Formspree with SSL encryption. We take reasonable measures to protect your personal information, though no internet transmission is completely secure.</p>
      </LegalSection>
      <LegalSection title="4. Cookies">
        <p>We use essential cookies to remember your age verification and site preferences. You may disable cookies in your browser settings, though some site features may not function properly as a result.</p>
      </LegalSection>
      <LegalSection title="5. Third Party Services">
        <p>We use Formspree for form processing and Vercel for website hosting. These services have their own privacy policies. We are not responsible for the privacy practices of third-party services.</p>
      </LegalSection>
      <LegalSection title="6. Contact">
        <p>For privacy-related questions contact us at <a href="mailto:trupepwellness@gmail.com" style={{color:GOLD}}>trupepwellness@gmail.com</a> or call 423-444-3668.</p>
      </LegalSection>
    </LegalPage>
  );
}
...
function TermsPage() {
  return (
    <LegalPage title="Terms of Service" lastUpdated="May 2025">
      <LegalSection title="1. Research & Educational Use Only">
        <p>ALL products sold by TruPep Wellness are strictly for research and educational purposes only. By accessing this website and purchasing products, you explicitly agree that you understand and accept this use restriction without exception.</p>
      </LegalSection>
      <LegalSection title="2. Age Requirement">
        <p>You must be 18 years of age or older to access this website and purchase products. By entering the site and completing a purchase, you confirm you meet this age requirement. TruPep Wellness reserves the right to cancel any order if age verification cannot be confirmed.</p>
      </LegalSection>
      <LegalSection title="3. Not Medical Advice">
        <p>Nothing on this website constitutes medical advice, medical diagnosis, or treatment recommendations. TruPep Wellness personnel are not licensed medical providers. You should always consult a qualified, licensed healthcare professional before beginning any wellness or research protocol.</p>
      </LegalSection>
      <LegalSection title="4. FDA Disclaimer">
        <p>Products have not been evaluated by the Food and Drug Administration. Products are not intended to diagnose, treat, cure, or prevent any disease or medical condition. Results are not guaranteed and will vary by individual.</p>
      </LegalSection>
      <LegalSection title="5. Assumption of Risk">
        <p>By purchasing products from TruPep Wellness, you acknowledge and accept all risks associated with the research use of peptide compounds. You agree to use all products responsibly and in accordance with applicable laws and regulations.</p>
      </LegalSection>
      <LegalSection title="6. Limitation of Liability">
        <p>TruPep Wellness, its owners, employees, and affiliates shall not be liable for any direct, indirect, incidental, consequential, or punitive damages arising from the purchase or use of our products or website. All purchases are made at the buyer's sole risk and discretion.</p>
      </LegalSection>
      <LegalSection title="7. Governing Law">
        <p>These Terms of Service are governed by the laws of the State of Florida. Any disputes arising from these terms or your use of our products or website shall be resolved in the courts of Miami-Dade County, Florida.</p>
      </LegalSection>
      <LegalSection title="8. Changes to Terms">
        <p>TruPep Wellness reserves the right to modify these terms at any time. Continued use of the website after any changes constitutes acceptance of the new terms.</p>
      </LegalSection>
    </LegalPage>
  );
}
...
function DisclaimerPage() {
  return (
    <LegalPage title="Research Use Disclaimer" lastUpdated="May 2025">
      <div style={{background:"rgba(201,168,76,0.06)",border:`1px solid rgba(201,168,76,0.25)`,padding:"20px 24px",marginBottom:32,borderLeft:`4px solid ${GOLD}`}}>
        <p style={{fontSize:14,color:WHITE,fontWeight:400,lineHeight:1.8}}>
          <strong>IMPORTANT:</strong> All peptide compounds, blends, and supplementary products offered by TruPep Wellness are intended strictly for <strong>research and educational purposes only</strong>. These products are NOT approved by the U.S. Food and Drug Administration for human consumption or therapeutic use.
        </p>
      </div>
      <LegalSection title="FDA Statement">
        <p>These statements have not been evaluated by the Food and Drug Administration. These products are not intended to diagnose, treat, cure, or prevent any disease or medical condition. Do not use these products as a substitute for professional medical advice, diagnosis, or treatment.</p>
      </LegalSection>
      <LegalSection title="Not Medical Advice">
        <p>The information provided on this website — including product descriptions, educational articles, protocol guides, stack recommendations, and the peptide calculator — is for informational and educational purposes only. It does not constitute medical advice, diagnosis, or treatment recommendations of any kind.</p>
        <p style={{marginTop:12}}>Always consult a licensed, qualified healthcare professional before beginning any wellness protocol, supplement regimen, or research program involving peptide compounds.</p>
      </LegalSection>
      <LegalSection title="Assumption of Risk">
        <p>Research peptide compounds carry inherent risks. By purchasing and using these products, you voluntarily assume all risks associated with their research use. TruPep Wellness strongly recommends working with a licensed healthcare provider when designing any peptide research protocol.</p>
      </LegalSection>
      <LegalSection title="Individual Results Vary">
        <p>Any results referenced on this website — including testimonials, client stories, and protocol outcomes — represent individual experiences and are not guaranteed, typical, or promised results. Individual outcomes will vary significantly based on health status, genetics, lifestyle, protocol adherence, and many other factors.</p>
      </LegalSection>
      <LegalSection title="Legal Compliance">
        <p>It is the sole responsibility of the purchaser to ensure that the research use of peptide compounds complies with all applicable local, state, federal, and international laws and regulations. TruPep Wellness makes no representations regarding the legal status of any compound in any jurisdiction.</p>
      </LegalSection>
      <LegalSection title="Professional Guidance">
        <p>We strongly recommend working with a qualified healthcare provider, physician, or licensed professional when designing and implementing any peptide research protocol. TruPep Wellness personnel are not licensed medical providers and cannot provide medical advice under any circumstances.</p>
      </LegalSection>
    </LegalPage>
  );
}
...
function RefundPage() {
  return (
    <LegalPage title="Refund & Shipping Policy" lastUpdated="May 2025">
      <LegalSection title="Order Processing">
        <p>Orders are typically processed within 1–3 business days of confirmed payment receipt. Processing times may vary during high-volume periods. You will be notified by email once your order has been processed and shipped.</p>
      </LegalSection>
      <LegalSection title="Shipping">
        <p>All orders are shipped with discreet, professional packaging. Shipping timeframes vary by carrier and destination. Tracking information will be provided where available. TruPep Wellness is not responsible for delays caused by carriers, customs, or circumstances outside our control.</p>
      </LegalSection>
      <LegalSection title="Refund Policy">
        <p>Due to the research nature of our products and strict quality control requirements, we do not accept returns or issue refunds on opened or used products. All sales are considered final once an order has been shipped.</p>
        <p style={{marginTop:12}}>We are committed to your satisfaction. If you believe there is an issue with your order, please contact us immediately.</p>
      </LegalSection>
      <LegalSection title="Damaged or Incorrect Orders">
        <p>If your order arrives damaged, defective, or incorrect, please contact us within <strong>48 hours of receipt</strong> with photos and your order details. We will work to resolve the issue promptly at no additional cost to you. Contact us at <a href="mailto:trupepwellness@gmail.com" style={{color:GOLD}}>trupepwellness@gmail.com</a> or 423-444-3668.</p>
      </LegalSection>
      <LegalSection title="Lost Packages">
        <p>If your tracking shows delivery but you have not received your package, please contact us within 7 days of the expected delivery date. We will work with you to investigate and resolve the issue.</p>
      </LegalSection>
      <LegalSection title="Payment">
        <p>TruPep Wellness accepts payment via Zelle (423-444-3668), Venmo (@TruPepWellness), and PayPal (@ItsTrinchy). Orders are processed once payment is confirmed. Please include your full name in the payment memo.</p>
      </LegalSection>
      <LegalSection title="Contact">
        <p>
          Email: <a href="mailto:trupepwellness@gmail.com" style={{color:GOLD}}>trupepwellness@gmail.com</a><br/>
          Phone/Text: <a href="tel:4234443668" style={{color:GOLD}}>423-444-3668</a><br/>
...
export default function App() {
  ...
  // Age gate check
  useEffect(() => {
    if (localStorage.getItem("tp_age_confirmed")) setAgeConfirmed(true);
    if (localStorage.getItem("tp_cookie_accepted")) setCookieAccepted(true);
  }, []);

  // Show popup after 5 seconds — only after age confirmed
  useEffect(() => {
  ...
  const acceptCookies = () => {
    localStorage.setItem("tp_cookie_accepted", "true");
  ...
  return (
    <>
      ...
      {/* Age gate — shows before anything else */}
