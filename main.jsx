...
const DIM2       = "rgba(255,255,255,0.38)";
...
function Nav({ count, onCart, setPage, page }) {
  ...
  return (
    <nav style={{
      ...
      {/* Mobile menu */}
      {mobile && open && (
        <div style={{ background:"rgba(10,10,10,.98)", borderTop:`1px solid ${BORDER}` }}>
          {LINKS.map(([l, id]) => (
            <button key={id} onClick={() => go(id)} style={{ display:"block", width:"100%", padding:"14px 28px", textAlign:"left", ...linkStyle(page === id) }}>{l}</button>
          ))}
        </div>
      )}
    </nav>
...
function Footer({ setPage }) {
  return (
    <footer style={{ borderTop:`1px solid ${BORDER}`, padding:"56px 24px 32px", marginTop:80, background:"rgba(5,5,5,.8)" }}>
      <div style={{ maxWidth:1200, margin:"0 auto" }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))", gap:44, marginBottom:48 }}>
          <div>
            ...
            <div style={{ fontSize:11, color:DIM2, lineHeight:1.9 }}>Miami's premier advanced peptide wellness source. Research compounds for the serious professional.</div>
          </div>
          <div>
            <div style={{ fontSize:9, letterSpacing:3, color:GOLD, textTransform:"uppercase", marginBottom:16 }}>Navigate</div>
            {LINKS.map(([l, id]) => (
              <button key={id} onClick={() => setPage(id)} style={{ display:"block", padding:"5px 0", color:DIM2, fontSize:11, background:"none", border:"none", cursor:"pointer", letterSpacing:1, transition:"color .2s" }}
                onMouseEnter={e => e.currentTarget.style.color = GOLD}
          ...
          <div>
            ...
            <div style={{ fontSize:11, color:DIM2, lineHeight:2.4 }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:14, paddingTop:8 }}>
          <p style={{ fontSize:10, color:"rgba(255,255,255,.2)", lineHeight:1.8, maxWidth:700 }}>
          ...
          <div style={{ fontSize:10, color:"rgba(255,255,255,.15)" }}>© {new Date().getFullYear()} TruPep Wellness</div>
        </div>
      </div>
    </footer>
  );
