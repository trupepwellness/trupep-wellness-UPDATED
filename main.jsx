...
// ── AUTH CONTEXT ────────────────────────────────────────────────
// Tracks the logged-in customer's Supabase Auth session plus their row
// in the `customers` table (name/email/phone). Any component can call
// useAuth() to know if someone's logged in, without re-fetching. Guest
// checkout still works fine when there's no session -- this context is
// purely additive.
const AuthContext = createContext({ session: null, customer: null, loading: true, refreshCustomer: () => {}, signOut: () => {} });
...
function AuthProvider({ children }) {
  ...
  async function loadCustomer(userId) {
    ...
    const { data, error } = await supabase.from("customers").select("*").eq("auth_user_id", userId).maybeSingle();
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      ...
      loadCustomer(data.session?.user?.id).then(() => setLoading(false));
    });
    const { data: sub } = supabase.auth.onAuthStateChange((_event, sess) => {
      setSession(sess);
      loadCustomer(sess?.user?.id);
    });
    return () => sub.subscription.unsubscribe();
  }, []);

  const refreshCustomer = () => loadCustomer(session?.user?.id);
  const signOut = () => supabase.auth.signOut();

  return (
    <AuthContext.Provider value={{ session, customer, loading, refreshCustomer, signOut }}>
      {children}
...
// ── ACCOUNT PAGE (page id "account") ─────────────────────────────
// Customer login/signup + order history. Uses the same Supabase Auth
// system as the admin login -- signups are public (unlike /admin), and
// a `customers` row gets created for each new account so the admin
// dashboard's Customers tab and order history both work automatically.
// Guest checkout still works fine for anyone who skips this entirely.
const ACCT_BORDER = "rgba(201,168,76,0.35)";
const ACCT_GLASS  = "rgba(255,255,255,0.05)";

function AccountPage() {
  const { session, customer, loading, refreshCustomer, signOut } = useAuth();

  if (loading) {
    return <div style={{ padding: "140px 24px", textAlign: "center", color: "rgba(255,255,255,0.6)" }}>Loading…</div>;
  }

  return session
    ? <AccountDashboard customer={customer} onSignOut={signOut} />
    : <AccountAuthForm onAuthed={refreshCustomer} />;
}

function AccountAuthForm({ onAuthed, onGuest, compact }) {
    ...
    if (mode === "signup") {
      ...
      if (userId) {
        await supabase.from("customers").insert({
          auth_user_id: userId, email: form.email, name: form.name, phone: form.phone,
        });
      }
      onAuthed();
    } else {
  ...
  return (
    <div style={{ padding: compact ? "0" : "120px 24px 80px", display: "flex", justifyContent: "center" }}>
      <div style={{ width: "100%", maxWidth: 400 }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 24, borderBottom: `1px solid ${ACCT_BORDER}` }}>
          {[["login", "Log In"], ["signup", "Create Account"]].map(([id, label]) => (
            <button key={id} onClick={() => { setMode(id); setErr(""); }} style={{
              ...
              color: mode === id ? GOLD : "rgba(255,255,255,0.5)",
        ...
        <form onSubmit={submit} style={{ padding: 28, border: `1px solid ${ACCT_BORDER}`, borderRadius: 8, background: ACCT_GLASS }}>
          <div style={{ fontFamily: SERIF, fontSize: 24, color: GOLD, marginBottom: 20, fontWeight: 300 }}>
            {mode === "login" ? "Welcome Back" : "Create Your Account"}
          </div>

          {mode === "signup" && (
            <>
              <input placeholder="Full Name" value={form.name} onChange={set("name")} required style={acctInput} />
              <input placeholder="Phone Number" value={form.phone} onChange={set("phone")} style={acctInput} />
            </>
          )}
          <input type="email" placeholder="Email Address" value={form.email} onChange={set("email")} required style={acctInput} />
          <input type="password" placeholder="Password" value={form.password} onChange={set("password")} required minLength={6} style={acctInput} />

          {err && <div style={{ color: "#e07a7a", fontSize: 12, marginBottom: 12 }}>{err}</div>}

          <button type="submit" disabled={busy} style={{
            width: "100%", padding: "13px", fontSize: 10, letterSpacing: 2, textTransform: "uppercase",
            fontWeight: 600, background: GOLD, color: BLACK, border: "none", borderRadius: 3, cursor: "pointer",
          }}>
          ...
          {onGuest ? (
            <button type="button" onClick={onGuest} style={{
              width: "100%", padding: "13px", marginTop: 10, fontSize: 10, letterSpacing: 2, textTransform: "uppercase",
              fontWeight: 600, background: "transparent", color: "rgba(255,255,255,0.7)", border: `1px solid ${ACCT_BORDER}`,
              borderRadius: 3, cursor: "pointer",
            }}>
              Continue as Guest →
            </button>
          ) : (
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginTop: 18, textAlign: "center", lineHeight: 1.7 }}>
              Prefer not to make an account? You can always{" "}
              <strong style={{ color: "rgba(255,255,255,0.8)" }}>check out as a guest</strong> from your cart.
            </div>
          )}
        </form>
      </div>
...
function AccountDashboard({ customer, onSignOut }) {
  ...
  useEffect(() => {
    ...
    supabase.from("orders").select("*").eq("customer_id", customer.id)
      ...
      .then(({ data, error }) => { if (!error) setOrders(data || []); setLoading(false); });
  }, [customer]);
    ...
    <div style={{ padding: "120px 24px 80px" }}>
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32, flexWrap: "wrap", gap: 12 }}>
          <div>
            <div style={{ fontFamily: SERIF, fontSize: 30, color: GOLD, fontWeight: 300 }}>
              Welcome{customer?.name ? `, ${customer.name.split(" ")[0]}` : ""}
            </div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.55)", marginTop: 4 }}>{customer?.email}</div>
          </div>
          <button onClick={onSignOut} style={{
            padding: "10px 20px", fontSize: 11, letterSpacing: 1.5, textTransform: "uppercase",
            background: "transparent", border: `1px solid ${GOLD}`, color: GOLD, borderRadius: 3, cursor: "pointer",
          }}>Log Out</button>
        </div>

        <div style={{ fontSize: 12, letterSpacing: 2, color: GOLD, textTransform: "uppercase", marginBottom: 16 }}>Order History</div>

        {loading && <div style={{ color: "rgba(255,255,255,0.55)", fontSize: 13 }}>Loading orders…</div>}
        {!loading && orders.length === 0 && (
          <div style={{ color: "rgba(255,255,255,0.55)", fontSize: 13, lineHeight: 1.8 }}>
            No orders yet on this account. Anything you order while logged in will show up here.
          </div>
        )}
        {!loading && orders.map((o) => (
          <div key={o.id} style={{ padding: "16px 20px", border: `1px solid ${ACCT_BORDER}`, borderRadius: 6, marginBottom: 12, background: ACCT_GLASS }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 8 }}>
              <span style={{ color: "rgba(255,255,255,0.6)" }}>{new Date(o.created_at).toLocaleDateString()}</span>
              <span style={{ color: GOLD, fontFamily: SERIF, fontSize: 16 }}>${Number(o.total).toFixed(2)}</span>
            </div>
            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", marginBottom: 6 }}>
              Payment: <span style={{ color: "rgba(255,255,255,0.8)" }}>{o.payment_status}</span>
              {" · "}Status: <span style={{ color: "rgba(255,255,255,0.8)" }}>{o.fulfillment_status}</span>
            </div>
            {(o.items || []).map((it, i) => (
              <div key={i} style={{ fontSize: 12, color: "rgba(255,255,255,0.75)" }}>
                {it.name} ({it.variant_label}) x{it.qty}
              </div>
            ))}
          </div>
        ))}
...
const acctInput = {
  ...
  border: `1px solid ${ACCT_BORDER}`, borderRadius: 4, background: "rgba(255,255,255,0.06)",
...
const G = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  @keyframes logoRingCCW{0%{transform:translate(-50%,-50%) rotate(0deg)}100%{transform:translate(-50%,-50%) rotate(-360deg)}}
  @keyframes heroBreathe{0%,100%{opacity:.55}50%{opacity:1}}
  ...
  .logo-ring-ccw{animation:logoRingCCW 9s linear infinite}
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", marginBottom:20 }}>
      ...
      <div style={{ position:"relative", animation:"float 3.5s ease-in-out infinite" }}>
        ...
        {/* Orbiting spark ring */}
        <div className="logo-ring-ccw" style={{
          position:"absolute", top:"50%", left:"50%",
          width:"112%", height:"112%",
          background:`conic-gradient(transparent 0deg,rgba(201,168,76,0.95) 2deg,rgba(255,252,200,1.0) 4deg,rgba(201,168,76,0.95) 6deg,transparent 9deg,transparent 360deg)`,
          borderRadius:"50%",
        ...
        <div className="logo-ring-ccw" style={{
          ...
          background:`conic-gradient(transparent 0deg,rgba(201,168,76,0.18) 0deg,rgba(201,168,76,0.52) 7deg,rgba(245,217,128,0.40) 12deg,rgba(201,168,76,0.18) 20deg,transparent 30deg,transparent 360deg)`,
          borderRadius:"50%",
        ...
        {/* 3D light source */}
        <div style={{ position:"absolute", top:"-18%", left:"-14%", width:"60%", height:"60%", background:"radial-gradient(circle, rgba(255,245,180,0.22) 0%, transparent 70%)", filter:"blur(18px)", pointerEvents:"none", zIndex:2 }} />
        <div style={{ position:"absolute", bottom:"-12%", right:"-10%", width:"55%", height:"55%", background:"radial-gradient(circle, rgba(0,0,0,0.55) 0%, transparent 70%)", filter:"blur(20px)", pointerEvents:"none", zIndex:2 }} />
        <div style={{ position:"absolute", inset:-50, borderRadius:"50%", background:"radial-gradient(circle, rgba(201,168,76,0.28) 0%, rgba(201,168,76,0.08) 48%, transparent 68%)", filter:"blur(20px)", pointerEvents:"none" }} />

        {/* Logo */}
        <img src={LOGO2} alt="TruPep Wellness" style={{
          width:200, height:200, objectFit:"contain",
          position:"relative", display:"block", zIndex:3,
          filter:["drop-shadow(0 0 36px rgba(240,200,74,0.75))","drop-shadow(-4px -6px 12px rgba(255,245,160,0.35))","drop-shadow(6px 10px 18px rgba(0,0,0,0.95))","drop-shadow(0 2px 6px rgba(0,0,0,0.80))"].join(" "),
        }} />
      </div>

      {/* Seam line */}
      <div style={{ width:140, height:1, marginTop:6, background:"linear-gradient(90deg, transparent, rgba(201,168,76,0.45), transparent)", flexShrink:0 }} />

      {/* Mirror — clipped to top 40% height so it can't overlap */}
      <div style={{ height:80, overflow:"hidden", flexShrink:0, lineHeight:0 }}>
        <div style={{ transform:"scaleY(-1)", transformOrigin:"top center" }}>
          <div style={{ animation:"floatMirror 3.5s ease-in-out infinite" }}>
            <img src={LOGO2} alt="" aria-hidden="true" style={{
              width:200, height:200, objectFit:"contain", display:"block",
              maskImage:"linear-gradient(to bottom, rgba(0,0,0,0.50) 0%, rgba(0,0,0,0.18) 30%, transparent 55%)",
              WebkitMaskImage:"linear-gradient(to bottom, rgba(0,0,0,0.50) 0%, rgba(0,0,0,0.18) 30%, transparent 55%)",
    ...
    <nav style={{
      ...
      <div style={{ maxWidth:1200, margin:"0 auto", padding:"0 28px", height:68, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        <div style={{ display:"flex", gap:14, alignItems:"center" }}>
          <button onClick={() => go("account")} title={session ? "My Account" : "Log In"} style={{
            position:"relative", width:40, height:40, borderRadius:"50%",
            border:`1px solid ${page === "account" ? GOLD : BORDER}`, display:"flex", alignItems:"center", justifyContent:"center",
            fontSize:16, transition:"all .2s", background:GLASS,
          }}
            onMouseEnter={e => { e.currentTarget.style.borderColor=GOLD; e.currentTarget.style.boxShadow=`0 0 14px ${GOLD_GLOW}`; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor=page === "account" ? GOLD : BORDER; e.currentTarget.style.boxShadow="none"; }}
          >
            👤
            {session && (
              <span style={{
                position:"absolute", bottom:-1, right:-1, width:10, height:10, borderRadius:"50%",
                background:"#7fd48a", border:`2px solid ${BLACK}`,
              }} />
            )}
          </button>
...
function EmailPopup({ onClose }) {
  ...
  const submit = async () => {
    ...
    const payload = {
      email,
      phone,
      _subject: "Your TruPep Wellness 10% Discount Code",
      _replyto: email,
      message: `Thank you for joining TruPep Wellness! Your exclusive discount code is: WELCOME10 — use it at checkout for 10% off your first order. Shop now at trupepwellness.com\n\nPhone provided: ${phone}`,
    };
    try {
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        {done ? (
          <div style={{ textAlign:"center" }}>
            ...
            <div style={{ fontSize:12, color:DIM, lineHeight:1.8 }}>Your code <strong style={{ color:GOLD }}>WELCOME10</strong> is ready at checkout. <span style={{ fontSize:10, color:DIM2 }}>Valid for first-time orders only. One use per customer.</span></div>
          </div>
        ) : (
          <>
            ...
            <div style={{ fontSize:10, color:DIM2, lineHeight:1.7, marginBottom:16, padding:"8px 12px", border:`1px solid rgba(201,168,76,0.2)`, borderRadius:4 }}>
              ⚠️ Valid for first-time orders only. One use per customer.
            </div>
            ...
            <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Your phone number" type="tel"
              ...
              onBlur={e => e.target.style.borderColor = BORDER}
            />
            {err && <div style={{ fontSize:10, color:"#ff6b6b", marginBottom:12 }}>{err}</div>}
...
function CartDrawer({ cart, total, onClose, onRemove, onQty, onAdd, setPage }) {
  ...
  const [form,     setForm]     = useState({ name:"", email:"", phone:"", street:"", city:"", state:"", zip:"", notes:"", contact_pref:"" });
  const [sending,  setSending]  = useState(false);
  const [submitError, setSubmitError] = useState("");

  // If logged in, prefill name/email/phone from the account -- still
  // fully editable, this is just a head start, not a lock.
  useEffect(() => {
    if (customer) {
      setForm((f) => ({
        ...f,
  ...
  const shippingFee = shipping === "overnight" ? 0 : SHIPPING_STANDARD;
  ...
  const submit = async () => {
    ...
    // If a promo code is applied, confirm this email hasn't already used
    // it on a previous order -- checked via a security-definer function so
    // guests (who can't read the orders table directly) can still be checked
    // without exposing anyone else's order data.
    if (discount > 0) {
      const code = promo.trim().toUpperCase();
      const { data: alreadyUsed, error: checkErr } = await supabase.rpc("check_promo_used", {
        p_email: form.email.trim(), p_code: code,
      });
      if (!checkErr && alreadyUsed) {
        setSubmitError(`The code ${code} has already been used with this email. Remove the code to continue, or use a different one.`);
        setSending(false);
        return;
      }
    }
    ...
    // and in the customer's own order history if they're logged in.
    try {
    ...
    const payload = {
      ...
      _replyto: form.email,
      order: orderLines,
      order_total: fmt(grandTotal),
      shipping_method: shipping === "overnight" ? "Overnight / Next Day (carrier quote will be provided)" : "Standard — $15.00",
    ...
    // Fallback: open mailto so order always gets through
    const body = encodeURIComponent(
      `NEW ORDER\n\nName: ${form.name}\nEmail: ${form.email}\nPhone: ${form.phone}\n` +
      `Address: ${form.street}, ${form.city}, ${form.state} ${form.zip}\n\n` +
      `ORDER:\n${orderLines}\n\nTotal: ${fmt(discounted)}` +
      (discount ? `\nDiscount: ${discount}%` : "") +
      (form.contact_pref ? `\nPreferred Contact: ${form.contact_pref}` : "") +
      (form.notes ? `\nNotes: ${form.notes}` : "")
    );
    const subject = encodeURIComponent(`New TruPep Order — ${form.name}`);
    window.location.href = `mailto:trupepwellness@gmail.com?subject=${subject}&body=${body}`;
    setSending(false);
    setStep("payment");
  };
        ...
        <div style={{ flex:1, padding:"24px 26px" }}>
              ...
              : <>
                  ...
                  <div style={{ borderTop:`1px solid ${BORDER}`, paddingTop:14, marginBottom:22 }}>
                    ...
                    <div style={{ marginBottom:14 }}>
                      <div style={{ fontSize:9, letterSpacing:2, color:GOLD, textTransform:"uppercase", marginBottom:8 }}>Shipping Method</div>
                      {[
                        ["standard", "Standard Processing & Shipping", "$15.00 flat rate · 3–5 business days"],
                        ["overnight", "Overnight / Next Day", "Rate quoted by carrier · contact us after checkout"],
                      ].map(([val, label, sub]) => (
                        <div key={val} onClick={() => setShipping(val)}
                          style={{
                            padding:"10px 14px", marginBottom:6, borderRadius:4, cursor:"pointer",
                            border:`1px solid ${shipping === val ? GOLD : BORDER}`,
                            background: shipping === val ? "rgba(240,200,74,0.08)" : GLASS,
                            transition:"all .18s",
                          }}>
                          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                            <div style={{ fontSize:11, color: shipping === val ? GOLD : WHITE, fontWeight:600 }}>{label}</div>
                            <div style={{ width:14, height:14, borderRadius:"50%", border:`2px solid ${shipping === val ? GOLD : BORDER}`, background: shipping === val ? GOLD : "none", flexShrink:0 }} />
                          </div>
                          <div style={{ fontSize:10, color:DIM2, marginTop:3 }}>{sub}</div>
                        </div>
                      ))}
                      {shipping === "overnight" && (
                        <div style={{ fontSize:10, color:"rgba(255,200,74,0.8)", padding:"8px 12px", background:"rgba(240,200,74,0.06)", border:`1px solid rgba(240,200,74,0.25)`, borderRadius:4, lineHeight:1.7 }}>
                          We will contact you after your order to confirm the carrier rate before shipping.
                        </div>
                      )}
                    </div>

                    <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, color:DIM2, marginBottom:4 }}>
                      <span>Shipping & Processing</span>
                      <span>{shipping === "overnight" ? "TBD" : fmt(SHIPPING_STANDARD)}</span>
                    </div>
                    <div style={{ display:"flex", justifyContent:"space-between", fontSize:15, fontWeight:600 }}>
                      <span>Total</span>
                      <span style={{ color:GOLD, fontFamily:SERIF }}>{shipping === "overnight" ? `${fmt(discounted)} + shipping` : fmt(grandTotal)}</span>
                    </div>
                  </div>
          ...
          {step === "checkout" && (
            <div style={{ animation:"fadeUp .3s ease" }}>
              ...
              {customer ? (
                <div style={{ fontSize:11, color:DIM, marginBottom:16, padding:"10px 14px", border:`1px solid ${BORDER}`, borderRadius:3, background:"rgba(201,168,76,.06)" }}>
                  Ordering as <strong style={{ color:GOLD }}>{customer.email}</strong> — this'll be saved to your order history.
                </div>
              ) : (
                <div style={{ fontSize:11, color:DIM2, marginBottom:16, padding:"10px 14px", border:`1px solid ${BORDER}`, borderRadius:3, background:GLASS }}>
                  Checking out as a guest. <button type="button" onClick={() => { onClose(); setPage("account"); }} style={{ color:GOLD, background:"none", border:"none", padding:0, cursor:"pointer", textDecoration:"underline" }}>Log in or create an account</button> to save your info and track orders — totally optional.
                </div>
              )}
              ...
              <div style={{ marginBottom:10 }}>
                <div style={{ fontSize:9, letterSpacing:2, color:GOLD, textTransform:"uppercase", marginBottom:8 }}>Preferred Contact Method</div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                  {[["📱 Text","Text"],["📧 Email","Email"],["📞 Call","Call"],["💬 WhatsApp","WhatsApp"]].map(([label, val]) => {
                    const selected = form.contact_pref === val;
                    return (
                      <button key={val} onClick={() => setForm(f => ({ ...f, contact_pref: val }))}
                        style={{
                          padding:"10px 8px", borderRadius:4, fontSize:11, fontWeight:500,
                          border:`1px solid ${selected ? GOLD : BORDER}`,
                          background: selected ? "rgba(240,200,74,0.12)" : GLASS,
              ...
              <div style={{ background:`rgba(201,168,76,.07)`, border:`1px solid ${BORDER}`, padding:"11px 15px", marginBottom:22, fontSize:11, color:DIM, borderRadius:3 }}>
                Total due: <strong style={{ color:GOLD }}>{shipping === "overnight" ? `${fmt(discounted)} + carrier shipping` : fmt(grandTotal)}</strong>
              </div>
          ...
          {step === "payment" && (
            <div style={{ animation:"fadeUp .3s ease" }}>
              ...
              <div style={{ fontSize:12, color:DIM, lineHeight:1.85, marginBottom:26 }}>
                Send <strong style={{ color:GOLD }}>{shipping === "overnight" ? `${fmt(discounted)} + carrier shipping` : fmt(grandTotal)}</strong> via any method below. Include your name in the memo.
              </div>
    ...
    <div style={{ borderRadius:8 }}>
      ...
      >
        ...
        <div style={{ borderTop:`1px solid ${BORDER}`, paddingTop:12, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          ...
          {product.coa_url ? (
            <a href={product.coa_url} target="_blank" rel="noopener noreferrer" style={{ fontSize:11, color:GOLD, fontWeight:600, display:"flex", alignItems:"center", gap:4 }}>
              View / Download COA
            </a>
          ) : (
            <span style={{ fontSize:11, color:"rgba(255,255,255,0.4)", fontStyle:"italic" }}>COA pending</span>
          )}
        </div>
      </div>
    </div>
...
// ── INTRO PAGE (page id "intro") ─────────────────────────────────
// First screen a visitor sees. Hosts the full ambient hero treatment
// (particles + the orbiting-ring logo) so the rest of the site can stay
// calmer -- this is the one place that gets to make an entrance.
function IntroPage({ setPage }) {
  const { session, customer, loading } = useAuth();

  return (
    <div style={{ position:"relative", minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"70px 24px", overflow:"hidden" }}>
      <FloatingParticles />
      <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse at 50% 42%, rgba(201,168,76,0.10) 0%, transparent 62%)", animation:"heroBreathe 7s ease-in-out infinite", pointerEvents:"none" }} />
      <div style={{ position:"relative", zIndex:1, textAlign:"center", maxWidth:420, width:"100%" }}>
        <FloatingHeroLogo />
        <div style={{ fontFamily:SERIF, fontSize:30, fontWeight:300, marginBottom:6 }}>TruPep Wellness</div>
        <div style={{ fontSize:9.5, letterSpacing:3, textTransform:"uppercase", color:DIM2, marginBottom:26 }}>Perform · Recover · Thrive</div>

        <div style={{ fontSize:12, color:DIM, lineHeight:1.9, marginBottom:24 }}>
          All products are sold strictly for <strong style={{ color:GOLD }}>research purposes only</strong> and are not intended for human consumption. You must be <strong style={{ color:GOLD }}>18 or older</strong> to enter.
        </div>

        {loading ? (
          <div style={{ color:DIM2, fontSize:12 }}>Loading…</div>
        ) : session ? (
          <>
            <div style={{ fontSize:13, color:DIM, marginBottom:18 }}>
              Welcome back{customer?.name ? `, ${customer.name.split(" ")[0]}` : ""}.
            </div>
            <Btn onClick={() => setPage("home")}>Enter Site</Btn>
          </>
        ) : (
          <AccountAuthForm compact onAuthed={() => setPage("home")} onGuest={() => setPage("home")} />
        )}
      </div>
    </div>
  );
...
function HomePage({ setPage, onAdd }) {
  ...
  return (
    <>
      {/* Hero */}
      <div style={{ padding:"110px 24px 60px", textAlign:"center", position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse at 50% 45%, rgba(201,168,76,0.10) 0%, transparent 62%)", animation:"heroBreathe 7s ease-in-out infinite", pointerEvents:"none" }} />
        <div style={{ position:"relative", zIndex:1 }}>
          <div style={{ fontSize:9, letterSpacing:6, color:GOLD, textTransform:"uppercase", marginBottom:24 }}>Miami's Premier Peptide Wellness</div>
...
// ── APP ─────────────────────────────────────────────────────────
// ── AI CHAT WIDGET ──────────────────────────────────────────────
// Reuses useState/useRef/useEffect and GOLD/BLACK/WHITE/SERIF/SANS already
// imported above — no separate imports needed. Talks to /api/chat.js, which
// pulls its product knowledge live from PRODUCTS/STACKS/CATEGORIES below.
function ChatWidget() {
  ...
  const [chatMessages, setChatMessages] = useState([
    {
      role: "assistant",
      content:
        "Hey! I'm the TruPep assistant. Ask me about our peptide categories, what they're commonly researched for, orders, or COAs — I'm here to help you find the right fit.",
    },
  ]);
  ...
  useEffect(() => {
    // Staggered well past the 4.5s WELCOME10 popup so the two never overlap.
    const t = setTimeout(() => setChatNudge(true), 9000);
  ...
  return (
    <>
      <style>{`
        .tp-chat-launcher { position: fixed; bottom: 24px; right: 24px; z-index: 9999; display: flex; align-items: center; gap: 12px; }
        .tp-chat-nudge { background: rgba(10,10,10,0.92); border: 1px solid ${GOLD}; color: ${WHITE}; font-family: ${SANS}; font-size: 13px; padding: 10px 16px; border-radius: 12px; white-space: nowrap; box-shadow: 0 4px 20px rgba(240,200,74,0.25); animation: tp-fade-in 0.4s ease; cursor: pointer; }
        @keyframes tp-fade-in { from { opacity: 0; transform: translateY(6px);} to { opacity: 1; transform: translateY(0);} }
        .tp-chat-button { width: 56px; height: 56px; border-radius: 50%; background: linear-gradient(135deg, ${GOLD}, #a8863a); border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 6px 24px rgba(240,200,74,0.4); transition: transform 0.2s ease; }
        .tp-chat-button:hover { transform: scale(1.06); }
        .tp-chat-panel { position: fixed; bottom: 96px; right: 24px; z-index: 9999; width: 360px; max-width: calc(100vw - 32px); height: 520px; max-height: calc(100vh - 140px); background: rgba(10,10,10,0.96); backdrop-filter: blur(16px); border: 1px solid rgba(240,200,74,0.35); border-radius: 18px; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 12px 48px rgba(0,0,0,0.6); animation: tp-fade-in 0.25s ease; }
        .tp-chat-header { padding: 16px 18px; border-bottom: 1px solid rgba(240,200,74,0.25); font-family: ${SERIF}; color: ${GOLD}; font-size: 20px; letter-spacing: 0.02em; display: flex; justify-content: space-between; align-items: center; }
        .tp-chat-close { background: none; border: none; color: ${WHITE}; opacity: 0.6; cursor: pointer; font-size: 18px; }
        .tp-chat-close:hover { opacity: 1; }
        .tp-chat-messages { flex: 1; overflow-y: auto; padding: 14px 16px; display: flex; flex-direction: column; gap: 10px; }
        .tp-msg { font-family: ${SANS}; font-size: 13.5px; line-height: 1.5; padding: 10px 13px; border-radius: 12px; max-width: 85%; }
        .tp-msg.assistant { background: rgba(240,200,74,0.12); border: 1px solid rgba(240,200,74,0.25); color: ${WHITE}; align-self: flex-start; }
        .tp-msg.user { background: ${GOLD}; color: ${BLACK}; align-self: flex-end; }
        .tp-chat-form { display: flex; gap: 8px; padding: 12px; border-top: 1px solid rgba(240,200,74,0.25); }
        .tp-chat-input { flex: 1; background: rgba(255,255,255,0.06); border: 1px solid rgba(240,200,74,0.3); border-radius: 10px; padding: 10px 12px; color: ${WHITE}; font-family: ${SANS}; font-size: 13px; outline: none; }
        .tp-chat-input:focus { border-color: ${GOLD}; }
        .tp-chat-send { background: ${GOLD}; border: none; border-radius: 10px; padding: 0 16px; font-family: ${SANS}; font-weight: 600; font-size: 13px; cursor: pointer; color: ${BLACK}; }
        .tp-chat-send:disabled { opacity: 0.5; cursor: default; }
        .tp-chat-disclaimer { font-family: ${SANS}; font-size: 10px; color: rgba(255,255,255,0.45); padding: 6px 16px 10px; text-align: center; line-height: 1.4; }
      `}</style>

      {chatOpen && (
        <div className="tp-chat-panel">
          <div className="tp-chat-header">
            <span>TruPep Assistant</span>
            <button className="tp-chat-close" onClick={() => setChatOpen(false)} aria-label="Close chat">✕</button>
          </div>
          <div className="tp-chat-messages" ref={chatScrollRef}>
            {chatMessages.map((m, i) => (
              <div key={i} className={`tp-msg ${m.role}`}>{m.content}</div>
            ))}
            {chatLoading && <div className="tp-msg assistant">Typing…</div>}
          </div>
          <form className="tp-chat-form" onSubmit={sendChatMessage}>
            <input
              className="tp-chat-input"
              value={chatInput}
              onChange={(e) => setChatInput(e.target.value)}
              placeholder="Ask about products, orders, COAs…"
            />
            <button className="tp-chat-send" type="submit" disabled={chatLoading || !chatInput.trim()}>Send</button>
          </form>
          <div className="tp-chat-disclaimer">
            Educational info only — not medical advice. Consult a licensed healthcare provider before use.
          </div>
        </div>
      )}
      ...
      <div className="tp-chat-launcher">
        {chatNudge && !chatOpen && (
          <div className="tp-chat-nudge" onClick={() => { setChatOpen(true); setChatNudge(false); }}>
            Need help or have questions? 👋
          </div>
        )}
        <button className="tp-chat-button" onClick={() => { setChatOpen((o) => !o); setChatNudge(false); }} aria-label="Open chat">
...
function App() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setPopup(true), 4500);
    return () => clearTimeout(t);
  }, [page === "intro"]);
  ...
  return (
    <>
      <style>{G}</style>
      {!isIntro && <GoldBorderFrame />}
      {!isIntro && <Nav count={count} onCart={() => setCartOpen(true)} setPage={setPageAndScroll} page={page} />}
      <main>
        {page === "intro"      && <IntroPage      setPage={setPageAndScroll} />}
      ...
      {!isIntro && <Footer setPage={setPageAndScroll} />}
      {cartOpen && <CartDrawer cart={cart} total={total} onClose={() => setCartOpen(false)} onRemove={remove} onQty={setQty} onAdd={add} setPage={setPageAndScroll} />}
      {popup && !isIntro && <EmailPopup onClose={() => setPopup(false)} />}
      {!isIntro && <ChatWidget />}
    </>
  );
}
