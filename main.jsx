...
function FloatingHeroLogo() {
  return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", marginBottom:44 }}>
      ...
      <div style={{
        width:150, height:1, marginTop:6,
        background:"linear-gradient(90deg, transparent, rgba(201,168,76,0.45), transparent)",
      }} />

      {/* ── Mirror reflection — sits cleanly below ── */}
      <div style={{
        transform:"scaleY(-1)",
        marginTop:2,
        lineHeight:0,
        ...
        <div style={{ animation:"floatMirror 3.5s ease-in-out infinite" }}>
          <img
            ...
            style={{
              ...
              maskImage:"linear-gradient(to bottom, rgba(0,0,0,0.55) 0%, rgba(0,0,0,0.25) 38%, rgba(0,0,0,0.08) 58%, transparent 72%)",
              WebkitMaskImage:"linear-gradient(to bottom, rgba(0,0,0,0.55) 0%, rgba(0,0,0,0.25) 38%, rgba(0,0,0,0.08) 58%, transparent 72%)",
              filter:"blur(1.5px) drop-shadow(0 0 8px rgba(201,168,76,0.30))",
            }}
          />
        </div>
      </div>
...
function SyringeViz({ units }) {
  ...
  // Barrel: needle at TOP. Fluid fills from needle end DOWN.
  // Scale: 0 at top, 100 at bottom. fillBottomY == tick position for exact unit.
  const barrelTop = 96;   // top of barrel (just below hub)
  const barrelBot = 340;  // bottom of barrel
  const barrelH   = barrelBot - barrelTop;
  // fillBottomY is EXACTLY where the unit tick is — no offsets
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:24, marginBottom:32 }}>
      <svg width="110" height="400" viewBox="0 0 110 400" style={{ overflow:"visible" }}>
        ...
        {/* Tip: starts at y=48, hub at y=76 = 28px ≈ 5/16" at syringe scale */}
        {/* Absolute tip — single pixel */}
        <line x1={cx} y1="48" x2={cx} y2="52" stroke="#fffce0" strokeWidth="0.5" opacity="0.95"/>
        {/* 32g body — tapers from 0.5px at tip to 2px at hub (very fine) */}
        <polygon
          points={`${cx-0.3},48 ${cx+0.3},48 ${cx+1.2},76 ${cx-1.2},76`}
          fill="url(#ndl)"
        />
        {/* Specular highlight */}
        ...
        {/* ── NEEDLE HUB ── */}
        <rect x={cx-9} y="76" width="18" height="18" rx="3"
          fill="url(#hub)" stroke="#C9A84C" strokeWidth="1.1"/>
        ...
        {/* ── FLUID — exact pixel alignment with ticks ── */}
        {fillPct > 0 && (
          <rect
            x={cx-18.5} y={barrelTop}
            width="37"
            height={fillBottomY - barrelTop}
            rx="0" fill="url(#fld)" opacity="0.90"
          />
        )}
        ...
        {fillPct > 0 && (
          <>
            <line x1={cx-24} y1={fillBottomY} x2={cx+24} y2={fillBottomY}
              stroke="#F0C84A" strokeWidth="2" strokeDasharray="5,3"/>
            <polygon
        ...
        {/* ── PLUNGER STOPPER — top face sits at fillBottomY ── */}
        <rect x={cx-19} y={fillBottomY} width="38" height="14" rx="4"
